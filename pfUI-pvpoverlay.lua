  ...
  function pfUI.uf:RefreshUnit(unit, component)
    ...
    if unitstr == "" or C.PvPOverlay.position == "off" then
      ...
      if pvp == 1 and faction == "Horde" then
        ...
        unit.dragonTop:Show()
        unit.dragonTop:SetVertexColor(.55,.09,.09,1)
        unit.dragonBottom:SetTexture(addonpath.."\\img\\BOTTOM_GRAY_"..pos)
        unit.dragonBottom:Show()
        unit.dragonBottom:SetVertexColor(.55,.09,.09,1)
      elseif pvp == 1 and faction == "Alliance" then
        unit.dragonTop:SetTexture(addonpath.."\\img\\TOP_GRAY_"..pos)
        unit.dragonTop:Show()
        unit.dragonTop:SetVertexColor(.18,.29,.58,1)
        unit.dragonBottom:SetTexture(addonpath.."\\img\\BOTTOM_GRAY_"..pos)
        unit.dragonBottom:Show()
        unit.dragonBottom:SetVertexColor(.18,.29,.58,1)
      else
        unit.dragonTop:Hide()
        unit.dragonBottom:Hide()
      end
